<td class="text-center fs-sm">
    ...
    <a class="btn btn-sm btn-alt-info" data-id="{{ $row->id }}" data-role-permissions="{{ $rolePermissions }}"
        data-bs-toggle="modal" data-bs-target="#role-permission-modal" data-bs-toggle="tooltip"
        aria-label="Role Permission" data-bs-original-title="Role Permission">
        <i class="fas fa-key"></i>
    </a>
    @endcan
    @can('role.edit')
    <a class="btn btn-sm btn-alt-secondary" data-id="{{ $row->id }}" data-name="{{ $row->name }}" data-bs-toggle="modal"
